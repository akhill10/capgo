...
Begin
  SELECT count(*)
  INTO is_found
  FROM public.decrypted_deleted_account
  WHERE decrypted_email=email_check;
  RETURN is_found = 0;
End; 
$$;
...
SECURITY LABEL FOR "pgsodium" ON COLUMN "public"."deleted_account"."email" IS 'ENCRYPT WITH KEY ID 8960523e-74df-4b7e-a62d-7c0be263fde7 SECURITY INVOKER';

DROP VIEW "public"."decrypted_deleted_account";
CREATE VIEW "public"."decrypted_deleted_account" WITH ("security_invoker"='true') AS
 SELECT "deleted_account"."created_at",
    "deleted_account"."id",
    "deleted_account"."email",
        CASE
            WHEN ("deleted_account"."email" IS NULL) THEN NULL::"text"
            ELSE
            CASE
                WHEN ('8960523e-74df-4b7e-a62d-7c0be263fde7' IS NULL) THEN NULL::"text"
                ELSE "convert_from"("pgsodium"."crypto_aead_det_decrypt"("decode"("deleted_account"."email", 'base64'::"text"), "convert_to"(''::"text", 'utf8'::"name"), '8960523e-74df-4b7e-a62d-7c0be263fde7'::"uuid", NULL::"bytea"), 'utf8'::"name")
            END
        END AS "decrypted_email"
   FROM "public"."deleted_account";

ALTER TABLE "public"."decrypted_deleted_account" OWNER TO "postgres";
...
CREATE OR REPLACE TRIGGER "deleted_account_encrypt_secret_trigger_email" BEFORE INSERT OR UPDATE OF "email" ON "public"."deleted_account" FOR EACH ROW EXECUTE FUNCTION "public"."deleted_account_encrypt_secret_email"();
...
GRANT ALL ON FUNCTION "public"."deleted_account_encrypt_secret_email"() TO "anon";
GRANT ALL ON FUNCTION "public"."deleted_account_encrypt_secret_email"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."deleted_account_encrypt_secret_email"() TO "service_role";
...
GRANT ALL ON TABLE "public"."decrypted_deleted_account" TO "anon";
GRANT ALL ON TABLE "public"."decrypted_deleted_account" TO "authenticated";
GRANT ALL ON TABLE "public"."decrypted_deleted_account" TO "service_role";
